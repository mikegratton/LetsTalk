...
{
    i_participant->subscribeWithId<reactor_progress>(
        reactorProgressName(i_serviceName),
...
/// Find the last reported progress state
...
std::unique_ptr<reactor_progress> ReactorClientBackend::progressData(Guid const& i_id, std::chrono::nanoseconds i_wait)
{
    LockGuard guard(m_mutex);
    auto it = m_session.find(i_id);
    if (it != m_session.end()) { return it->second.progressData.pop(i_wait); }
    return nullptr;
}
