...
class ThreadSafeQueue : public Awaitable {
   public:
     ...
     * return nullptr.
     *
     * @param i_waitUntil Wait no longer than the given timepoint
     */
    std::unique_ptr<T> pop(std::chrono::steady_clock::time_point i_waitUntil)
    {
        LockGuard guard(m_mutex);
        if (m_nonempty.wait_until(guard, i_waitUntil, [this]() { return !m_queue.empty(); })) {
            auto popped = std::move(m_queue.front());
            m_queue.pop_front();
            return popped;
        }
     ...
     * @param i_waitUntil Wait no longer than the given timepoint
     */
    Queue popAll(std::chrono::steady_clock::time_point i_waitUntil)
    {
        Queue queue;
        LockGuard guard(m_mutex);
        if (m_nonempty.wait_until(guard, i_waitUntil, [this]() { return !m_queue.empty(); })) { queue.swap(m_queue); }
        return queue;
    }
